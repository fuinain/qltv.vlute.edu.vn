...
export default {
  ...
  methods: {
    async fetchData(page = 1) {
      try {
        const response = await axios.get(`/api/donvi?page=${page}`);
    ...
    async themPhongKhoa() {
      ...
      try {
        const response = await axios.post("/api/donvi", this.DonVi);
        if (response.data.status === 200) {
    ...
    async suaPhongKhoa(row) {
      ...
      try {
        const response = await axios.put(`/api/donvi/${row.id_don_vi}`, this.DonVi);
        if (response.data.status === 200) {
          toastr.success(response.data.message);
          this.fetchData(this.currentPage);
          this.$refs.modal.closeModal();
    ...
    xoaPhongKhoa(row) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          try {
            const response = await axios.delete(`/api/donvi/${row.id_don_vi}`);
            if (response.data.status === 200) {
              toastr.success(response.data.message);
              this.fetchData(this.currentPage);
            }
