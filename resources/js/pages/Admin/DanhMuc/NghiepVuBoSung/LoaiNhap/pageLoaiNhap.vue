<template>
    <ContentWrapper title="QUẢN LÝ LOẠI NHẬP">
        <template #ContentPage>
            <div class="row">
                <div class="col-12">
                    <Card>
                        <template #ContentCardHeader>
                            <button type="button" class="btn btn-primary" @click="themLoaiNhap">
    ...
    <Modal ref="modal">
        <div class="row">
            <div class="col-md-12">
                <Input v-model="LoaiNhap.ten_loai_nhap" label="Tên loại nhập" placeholder="Tên loại nhập..." type="text"/>
            </div>
        </div>
    </Modal>
</template>
...
export default {
    ...
    data() {
        return {
            ...
            headers: [
                ...
                {key: 'ten_loai_nhap', label: 'Loại nhập'},
    ...
    methods: {
        async fetchData(page = 1) {
            try {
                const response = await axios.get(`/api/danh-muc/nghiep-vu-bo-sung/loai-nhap?page=${page}`);
        ...
        async themLoaiNhap() {
            ...
            try {
                const response = await axios.post("/api/danh-muc/nghiep-vu-bo-sung/loai-nhap", this.LoaiNhap);
                if (response.data.status === 200) {
        ...
        async suaLoaiNhap(row) {
            ...
            try {
                const response = await axios.put(`/api/danh-muc/nghiep-vu-bo-sung/loai-nhap/${row.id_loai_nhap}`, this.LoaiNhap);
                if (response.data.status === 200) {
                    toastr.success(response.data.message);
                    this.fetchData(this.currentPage);
                    this.$refs.modal.closeModal();
        ...
        async xoaLoaiNhap(row) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    try {
                        const response = await axios.delete(`/api/danh-muc/nghiep-vu-bo-sung/loai-nhap/${row.id_loai_nhap}`);
                        if (response.data.status === 200) {
                            toastr.success(response.data.message);
                            this.fetchData(this.currentPage);
                        }
