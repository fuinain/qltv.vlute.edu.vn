<template>
    <ContentWrapper title="DANH MỤC KHO ẤN PHẨM">
        <template #ContentPage>
            <div class="row">
                <div class="col-12">
                    <Card>
                        <template #ContentCardHeader>
                            <button type="button" class="btn btn-primary" @click="themDMKhoAnPham">
...
export default {
    ...
    methods: {
        async fetchData(page = 1) {
            try {
                const response = await axios.get(`/api/quan-ly-an-pham/kho-an-pham/danh-muc-kho?page=${page}`);
        ...
        async themDMKhoAnPham() {
            ...
            // Kiểm tra validate
            if (
                !this.DMKhoAnPham.ma_kho.trim() ||
                !this.DMKhoAnPham.ten_kho.trim()
            ) {
            ...
            try {
                const response = await axios.post("/api/quan-ly-an-pham/kho-an-pham/danh-muc-kho", this.DMKhoAnPham);
                if (response.data.status === 200) {
        ...
        async suaDMKhoAnPham(row) {
            ...
            try {
                const response = await axios.put(`/api/quan-ly-an-pham/kho-an-pham/danh-muc-kho/${row.id_kho_an_pham}`, this.DMKhoAnPham);
                if (response.data.status === 200) {
                    toastr.success(response.data.message);
                    this.fetchData(this.currentPage);
                    this.$refs.modal.closeModal();
        ...
        async xoaDMKhoAnPham(row) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    try {
                        const response = await axios.delete(`/api/quan-ly-an-pham/kho-an-pham/danh-muc-kho/${row.id_kho_an_pham}`);
                        if (response.data.status === 200) {
                            toastr.success(response.data.message);
                            this.fetchData(this.currentPage);
                        }
